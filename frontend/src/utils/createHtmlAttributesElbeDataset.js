...
export const createHtmlAttributesElbeDataset = (rootState, dispatch, zoomLng, zoomLat, list, wordlist, duplicated, popup) =>{
    ...
    const fullTxtBtn = document.createElement('button');
    fullTxtBtn.setAttribute('class', 'btn btn-outline-success btn-sm mt-2');
    fullTxtBtn.style.cssText = 'float:right'
    fullTxtBtn.textContent = 'Full Text';
    zoomDiv.appendChild(fullTxtBtn)
    ...
    listbtn.onclick = function(){
        popup.remove()
        dispatch('geoparsing/backtoDuplicatedListElbe',null , { root:true });
    }

    fullTxtBtn.onclick = function() {
        let html = list.full_text
        console.log(html)
        let tab = window.open("", '_blank')
        tab.document.write('<p>'+html+'</p>')
        tab.document.close()
    }
