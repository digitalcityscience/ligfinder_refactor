...
def delete_item_from_user_history():
    if request.method == 'POST':
        data = request.get_json()
        delete_item_user_history(
            data["id"], data["id"], data["id"], data["deleteItemName"])
    return "ok"
...
def edit_item_from_user_history():
    if request.method == 'POST':
        ...
        desc = f'"{desc}"'
        update_user_history_item_description(
            data["payload"]["name"], desc, data["payload"]["id"])
    return "ok"
